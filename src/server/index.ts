...
salesforce.pricebookEntries$.subscribe(entries =>
  entries.forEach(entry =>
    mail
      .upsertProduct(entry)
      .subscribe(() => console.log("[LOG] Updated product in Mailchimp"))
  )
);
...
app.get("/api/v1/reservation/:id", (req, res) => {
  ...
  observableCombineLatest(
    ...
    salesforce.getCampaign(opportunity.CampaignId).subscribe(campaign => {
      res.json({
        opportunity,
        contact,
        campaign
...
app.get("/api/v1/reservation/:id/campaign", (req, res) => {
  ...
  salesforce
    .getOpportunity(id)
    .pipe(
      switchMap(opportunity => salesforce.getCampaign(opportunity.CampaignId))
    )
...
app.get("/api/v1/reservation/:id/pay", (req, res) => {
  ...
  salesforce.getOpportunity(id).subscribe(opportunity => {
  ...
  RxHR.post(SETTINGS.salesforce.endpoints.reservation, {
    ...
    if (data.response.statusCode != 201) {
      ...
      observableCombineLatest(
        ...
        .pipe(
          mergeMap(([contact, lines, entries]) =>
            mail.insertReservation(
              reservation,
              opportunity,
              contact,
              lines,
