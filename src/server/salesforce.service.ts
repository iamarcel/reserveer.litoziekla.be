...
const CAMPAIGN_FIELDS =
  "Id,Name,Maximum_Opportunities__c,MaximumProducts__c,Hero_Image__c," +
...
const PRICEBOOKENTRY_FIELDS =
  "Id,Name,Pricebook2Id,Product2Id," +
...
export default class SalesforceService {
  ...
  private _login$: Observable<Connection>;
  ...
  constructor() {
    ...
    this._login$.subscribe(
      _ => console.log("[LOG] Logged in to Salesforce."),
    ...
    this.production$.subscribe(
      _ => console.log("[LOG] Production updated."),
      err => console.error("[ERR] while caching production:\n", err)
    );

    // Refresh sponsors & production
    this.sponsors$ = observableCombineLatest(
      this._login$,
      this.production$
    ).pipe(mergeMap(this.getSponsors), publishReplay(1), refCount());
    ...
    this.pricebookEntries$.subscribe(
      _ => console.log("[LOG] Products updated."),
  ...
  connectProductionObservable(
    refresh$: Observable<any>,
    subject: Subject<Campaign>
  ): void {
    observableCombineLatest(this._login$, refresh$).subscribe(
      ([connection, _]) => {
        ...
        connection
          ...
          .execute({}, (err: Error, record: any) => {
            // Typedef for record incorrectly shows array
            console.log("[LOG] Received answer about current production.");
            if (err) {
              return subject.error(err);
            }

            let campaign = record as Campaign;
            let html = campaign.Hero_Image__c;
            campaign.Hero_Image__c = (html || "").replace(
              /--c\.documentforce\.com/,
              ".secure.force.com/test"
            );
  ...
  getPricebookEntries(
    ...
    return Observable.create((observer: Observer<PricebookEntry[]>) => {
      connection
        ...
        .execute({}, (err, results: any) => {
          if (err) {
            return observer.error(err);
          }
          observer.next(results as PricebookEntry[]);
          observer.complete();
        });
    });
  }
        ...
        return Observable.create((observer: Observer<Contact>) => {
          this.connection
            ...
            .execute({}, (err, result: any) => {
              ...
              observer.next(result as Contact);
              observer.complete();
            });
        });
      })
    ...
    return Observable.create((observer: Observer<Campaign>) =>
      this.connection
        ...
        .execute({}, (err, result: any) => {
          ...
          observer.next(result as Campaign);
          observer.complete();
        })
    );
  }
  ...
  ): Observable<OpportunityLineItem[]> {
    return Observable.create((observer: Observer<OpportunityLineItem[]>) =>
      this.connection
        ...
        .execute({}, (err, results: any) => {
          ...
          observer.next(results as OpportunityLineItem[]);
          observer.complete();
        })
    );
  }
  ...
  countTotalQuantity() {
    return this.request<CampaignQuantityCount[]>(
      this.connection.query(
        "SELECT CampaignId, SUM(TotalOpportunityQuantity) TotalQuantity " +
    ...
    return this.countTotalQuantity().pipe(
      map((quantities: CampaignQuantityCount[]) => {
        campaign.ChildCampaigns = campaign.ChildCampaigns.map(childCampaign => {
          const quantityForThisCampaign = quantities.find(
            q => q.CampaignId == childCampaign.Id
          );
  ...
  postMessage(text: string, subjectId: string) {
    (this.connection as any).chatter.resource("/feed-elements").create(
      {
        body: {
          messageSegments: [
            {
