...
export default class Mail {
  ...
  public addCustomer(contact: Contact) {
    return this.request.post(`${this.storePath}/customers`, {
      body: {
        id: contact.Id,
  ...
  public upsertProduct(entry: PricebookEntry) {
    return this.request
      .get(`${this.storePath}/products/${entry.Product2.Id}`)
      .pipe(
        switchMap((data: any) => {
          ...
          if (data.response.statusCode == 200) {
            ...
            return this.request.post(`${this.storePath}/products`, {
              body: mcProduct
            });
          }
        })
      ...
      this.request
        .post(`${this.storePath}/orders`, {
          body: {
            ...
            lines: lines.map(l => ({
              id: l.Id,
              product_id: entries.find(entry => entry.Id === l.PricebookEntryId)
                .Product2Id,
        ...
        .pipe(
          tap(data =>
            console.log(
              `[LOG] Added opportunity ${opportunity.Id} in Mailchimp`
            )
          )
        ),
  ...
  public confirmPayment(orderId: string) {
    return this.request.patch(`${this.storePath}/orders/${orderId}`, {
      body: {
